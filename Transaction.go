...
/*
peer chaincode invoke -n mycc3 -c '{"Args":["submitApproveTransaction", "B","004000000001" , "002000000001" , "A07103" , "102000","100000","true","BANK004B00400000000120180415070724","BANKCBC"]}' -C myc -v 9.0
peer chaincode invoke -n mycc3 -c '{"Args":["submitApproveTransaction", "S","002000000001" , "004000000001" , "A07103" , "102000","100000","true","BANK002S00200000000120180415065316","BANKCBC"]}' -C myc -v 9.0
...
func (s *SmartContract) submitApproveTransaction(
	...
	if len(args[0]) <= 0 {
		return shim.Error("TXType must be a non-empty string")
	}
	if len(args[1]) <= 0 {
		return shim.Error("TXFrom must be a non-empty string")
	}
	if len(args[2]) <= 0 {
		return shim.Error("TXTo must be a non-empty string")
	}
	if len(args[3]) <= 0 {
	...
	if len(args[5]) <= 0 {
		return shim.Error("Payment must be a non-empty string")
	}
	if len(args[6]) <= 0 {
		return shim.Error("isApproved flag must be a non-empty string")
	}
	if len(args[7]) <= 0 {
	...
	if (TXType != "B") && (TXType != "S") {
		return shim.Error("TXType must be a B or S")
	}
	TXFrom := strings.ToUpper(args[1])
	TXTo := strings.ToUpper(args[2])
	BankFrom := SubString(TXFrom, 0, 3)
	BankTo := SubString(TXTo, 0, 3)
	...
	if err != nil {
		...
		return shim.Error("SecurityAmount must be a positive value")
	}
	Payment, err := strconv.ParseInt(args[5], 10, 64)
	if err != nil {
		return shim.Error("Payment must be a numeric string")
	} else if Payment < 0 {
		return shim.Error("Payment must be a positive value")
	}
	isApproved, err := strconv.ParseBool(strings.ToLower(args[6]))
	if err != nil {
		return shim.Error("isApproved must be a boolean string")
	}
	...
	if isApproved != true {
		err := updateQueuedTransactionApproveStatus(stub, TXKEY, TXID, MatchedTXID, "PaymentError")
		if err != nil {
			return shim.Error(err.Error())
		}

		err = updateHistoryTransactionApproveStatus(stub, HTXKEY, TXID, MatchedTXID, "PaymentError")
		if err != nil {
			return shim.Error(err.Error())
		}

		err = updateTransactionStatus(stub, TXID, "PaymentError", "")
		if err != nil {
			return shim.Error(err.Error())
		}
		err = updateTransactionStatus(stub, MatchedTXID, "PaymentError", "")
		if err != nil {
			return shim.Error(err.Error())
		}
		...
		if TXType == "S" {
			...
			err = updateBankTotals(stub, BankFrom, SecurityID, Payment, false)
			if err != nil {
				return shim.Error(err.Error())
			}
		...
		if TXType == "B" {
			...
			err = updateBankTotals(stub, BankTo, SecurityID, Payment, true)
			if err != nil {
				return shim.Error(err.Error())
			}
			if (senderBalance < 0) || (receiverBalance < 0) {
		...
		err := updateQueuedTransactionApproveStatus(stub, TXKEY, TXID, MatchedTXID, "Finished")
		if err != nil {
			return shim.Error(err.Error())
		}

		err = updateHistoryTransactionApproveStatus(stub, HTXKEY, TXID, MatchedTXID, "Finished")
		if err != nil {
			return shim.Error(err.Error())
		}

		err = updateTransactionStatus(stub, TXID, "Finished", MatchedTXID)
		if err != nil {
			return shim.Error(err.Error())
		}

		err = updateTransactionStatus(stub, MatchedTXID, "Finished", TXID)
		if err != nil {
			return shim.Error(err.Error())
		}
...
func (s *SmartContract) submitEndDayTransaction(
	...
	err = updateEndDayQueuedTransactionStatus(stub, TXKEY, TXID)
	if err != nil {
		return shim.Error(err.Error())
	}
	err = updateEndDayHistoryTransactionStatus(stub, HTXKEY, TXID)
	if err != nil {
		return shim.Error(err.Error())
	}
	err = updateEndDayTransactionStatus(stub, TXID)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (s *SmartContract) securityTransfer(
	...
	TimeNow := time.Now().Format(timelayout)
	newTX, isPutInQueue, err := validateTransaction(stub, args)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func validateTransaction(
	stub shim.ChaincodeStubInterface,
	args []string) (Transaction, bool, error) {
	TimeNow := time.Now().Format(timelayout)
	var err error
	...
	if len(args[0]) <= 0 {
		return transaction, false, errors.New("TXType must be a non-empty string")
	}
	if len(args[1]) <= 0 {
		return transaction, false, errors.New("TXFrom must be a non-empty string")
	}
	if len(args[2]) <= 0 {
		return transaction, false, errors.New("TXTo must be a non-empty string")
	}
	if len(args[3]) <= 0 {
		return transaction, false, errors.New("SecurityID must be a non-empty string")
	}
	if len(args[4]) <= 0 {
		return transaction, false, errors.New("SecurityAmount must be a non-empty string")
	}
	if len(args[5]) <= 0 {
		return transaction, false, errors.New("Payment must be a non-empty string")
	}
	if len(args[6]) <= 0 {
		return transaction, false, errors.New("isPutToQueue flag must be a non-empty string")
	}
	isPutToQueue, err := strconv.ParseBool(strings.ToLower(args[6]))
	if err != nil {
		return transaction, false, errors.New("isPutToQueue must be a boolean string")
	}
	transaction.isPutToQueue = isPutToQueue
	TXType := SubString(strings.ToUpper(args[0]), 0, 1)
	if (TXType != "B") && (TXType != "S") {
		return transaction, false, errors.New("TXType must be a B or S")
	}
	...
	if TXFrom == TXTo {
		return transaction, false, errors.New("TXFrom equal to TXTo.")
	}
	...
	if err != nil {
		return transaction, false, errors.New("SecurityID does not exits.")
	}
	transaction.SecurityID = SecurityID
	SecurityAmount, err := strconv.ParseInt(args[4], 10, 64)
	if err != nil {
		return transaction, false, errors.New("SecurityAmount must be a numeric string")
	} else if SecurityAmount < 0 {
		return transaction, false, errors.New("SecurityAmount must be a positive value")
	}
	transaction.SecurityAmount = SecurityAmount
	Payment, err := strconv.ParseInt(args[5], 10, 64)
	if err != nil {
		return transaction, false, errors.New("Payment must be a numeric string")
	} else if Payment < 0 {
		return transaction, false, errors.New("Payment must be a positive value")
	}
	...
	return transaction, isPutToQueue, nil
...
func (s *SmartContract) securityCorrectTransfer(
	...
	TimeNow := time.Now().Format(timelayout)
	newTX, isPutInQueue, err := validateCorrectTransaction(stub, args)
	if err != nil {
		return shim.Error(err.Error())
	}
	TXIndex := newTX.TXIndex
...
func validateCorrectTransaction(
	...
	if len(args[7]) <= 0 {
		return transaction, false, errors.New("TXID flag must be a non-empty string")
	}
	TXID = strings.ToUpper(args[7])
	sourceTX, err := getTransactionStructFromID(stub, TXID)
	if sourceTX.TXStatus != "Pending" {
		return transaction, false, errors.New("Failed to find Transaction Pending TXStatus")
	}
...
func updateEndDayTransactionStatus(stub shim.ChaincodeStubInterface, TXID string) error {

	TimeNow := time.Now().Format(timelayout)
	transaction, err := getTransactionStructFromID(stub, TXID)
	if transaction.TXStatus != "Pending" {
		return errors.New("Failed to find Transaction Pending TXStatus")
	}
...
func updateEndDayQueuedTransactionStatus(stub shim.ChaincodeStubInterface, TXKEY string, TXID string) error {
	...
	if doflg != true {
		return errors.New("Failed to find Queued Pending TXStatus ")
	}
...
func updateEndDayHistoryTransactionStatus(stub shim.ChaincodeStubInterface, HTXKEY string, TXID string) error {
	...
	for key, val := range historyTX.TXIDs {
		...
		if val == TXID {
			fmt.Printf("hkey2: %d\n", key)
			fmt.Printf("hval2: %s\n", val)
			if historyTX.Transactions[key].TXStatus == "Pending" {
				historyTX.Transactions[key].TXStatus = "Cancelled"
				historyTX.Transactions[key].UpdateTime = TimeNow
				historyTX.TXStatus[key] = "Cancelled"
				doflg = true
	...
	if doflg != true {
		return errors.New("Failed to find History Pending TXStatus ")
	}
